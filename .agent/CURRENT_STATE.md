...
| ACS-P0-004 | **REJECTED (Codex) — fix runda u toku, NE MERGE** | Crush | Codex, Claude | HEAD i dalje `d379813`. Codex review: `agent_reports/2026-08-31-ACS-P0-004-review-codex.md` — 3 blocking findings, sva potvrđena nezavisno od koordinatora: **BF-1** prazan `formats:` YAML ključ baca `TypeError` umjesto `RegistryError` (`raw.get("formats", [])` ne hvata `None` vrijednost postojećeg ključa); **BF-2** cache-ovani Pydantic modeli su `frozen=True` ali sadrže mutable `list` polja — caller može mutirati vraćenu `PlatformDefinition` i pokvariti registry state za sve naredne pozive (stvaran shared-mutable-state bug); **BF-3** `supported_formats` sa case-varijantom duplikata (`[STORY, story]`) prolazi normalizaciju bez duplicate-check, `list_formats()` vraća duplirane kodove. Fix brief: `agent_reports/2026-08-31-ACS-P0-004-fix-round-brief.md` — BF-2 fix predlaže `tuple[str,...]` umjesto `list[str]` (isti obrazac kao ACS-P0-003). |
