...
**Zadnje ažurirano:** 2026-09-02 (coordinator: claude) — **ACS-GUI-001 GOTOV — merged u main** (`cad003e`→`9259792`, 346 testova, ruff/mypy/import-boundaries čisti). Round 2 fix: sva tri nalaza iz round 1 review-a stvarno riješena — `static/app.css`/`app.js` su sada bajt-identične kopije `docs/gui-v3/shared/` (SHA-256 provjereno), `.lang-toggle` uklonjen, sva 4 placeholder ekrana (Brend/Kampanje/Kalendar/Podešavanja) + Početna sad genuinely prolaze kroz jedan `render_shell()` (`screens/_static_pages.py` `write_all_pages()`, DRY-enforcement test). Koordinator dodatno live-pokrenuo `python -m ai_campaign_studio.presentation_webview` na ovoj mašini — proces log potvrđuje pravi `Chrome_WidgetWin` (Edge WebView2) prozor, ne mshtml fallback. Prvi produkcijski GUI kod je sad na `main`-u: shell + Početna (fixture-wired) + 4 placeholder ekrana koji čekaju **ACS-GUI-002**. `docs/gui-v3/` kanonski GUI izvor, `docs/PYWEBVIEW_SECURITY.md` hardening politika, G9 gate zatvoren. Sljedeći Faza 1 domain task: A6 (fixture loading kroz repository sloj); sljedeći GUI task: ACS-GUI-002 (preostalih 8 ekrana).
