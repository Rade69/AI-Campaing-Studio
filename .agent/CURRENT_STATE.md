...
**Zadnje ažurirano:** 2026-09-02 (coordinator: claude) — **ACS-F1-010 merged u main (HIGH risk, puni
...
**A3–A10 su GOTOVI** — domain sloj (ACS-F1-001/002), boundary schemas (ACS-F1-003/004),
...
(ACS-F1-009), SocialPostPayload persistence (ACS-F1-010, HIGH) svi merged u main (2026-09-02),
455 testova, sve zeleno. GUI paralelno: svih 5 sidebar ekrana (Početna + Brend/Kampanje/
Kalendar/Podešavanja) takođe merged i **live-verifikovano od Human Owner-a**.

**ACS-F1-011 UNBLOCKED, Pi već u toku** (`GenerateSocialPost`, worktree sinhronizovan sa main-om
nakon ACS-F1-010 merge-a, `application/posts/select_allowed_facts.py` +
`application/posts/claim_validator.py` necommit-ovano u progresu). Ostali kandidati, još bez
kontrakta: **A10 — Plan editing/versioning/approval** (edit/reorder/approve nad `CampaignPlan`,
plan sekcija "A10" — NAPOMENA: ovo "A10" je plan-numeracija, različito od task-ID-a ACS-F1-010
koji je već DONE), **A8 — live provider adapter** (i dalje odgođen po Human Owner odluci,
