...
| ACS-P0-005 | **OPEN — contract spreman, worktree+branch kreirani, čeka implementaciju (Pi)** | Pi | Codex, Claude (elevated §4 — SecretStore) | Scope: P0.14–P0.15. Kontrakt: `agent_reports/ACS-P0-005-task-contract.md`. Worktree `../ai-campaign-studio-worktrees/ACS-P0-005-ai-registry-secrets`, branch `task/ACS-P0-005-ai-registry-secrets` od `main@820bbf9`. Paralelno sa 006, `allowed_paths` disjoint potvrđeno. `adversarial_required: true` (security-kritično — secret ne smije procuriti u log/exception repr). |
| ACS-P0-006 | IMPLEMENTED, Claude review PASS, čeka Codex review + Human Owner approval | Crush | Codex, Claude | Commit `92f3917` na `task/ACS-P0-006-sqlite-foundation`. Crush nije predao self-report (isti obrazac kao 001/004) — koordinator rekonstruisao evidence i sam izvršio oba adversarial dokaza (checksum-mismatch, failure-rollback). Evidence: `agent_reports/2026-09-01-ACS-P0-006-crush-confirmed.md`. Claude review: `agent_reports/2026-09-01-ACS-P0-006-review-claude.md` (PASS). Codex brief: `agent_reports/2026-09-01-ACS-P0-006-codex-review-request.md` — uključuje pitanje o mogućem CRLF/LF checksum mismatch riziku (repo koristi autocrlf). Napomena: fresh worktree `.venv` je opet imao oštećen pydantic_core wheel (isto kao ACS-P0-004), popravljeno reinstall-om. |
