...
**A3–A12(dio 1) + A10(plan-numeracija) su GOTOVI** — domain sloj (ACS-F1-001/002), boundary
schemas (ACS-F1-003/004), business persistence (ACS-F1-005/006), fixture load use-case
...
versioning/approval (ACS-F1-014) svi merged u main (2026-09-02), 512 testova, sve zeleno. GUI
paralelno: svih 5 sidebar ekrana (Početna + Brend/Kampanje/Kalendar/Podešavanja) takođe merged i
**live-verifikovano od Human Owner-a**.
...
CreateCampaign → GenerateCampaignPlan → **EditCampaignPlan/ReorderCampaignItem/
ApproveCampaignPlan** → GenerateSocialPost → claim_linter/derive_content_status, svaki korak
atomičan, svaki nezavisno testiran I lančano integration-testiran preko pravih SQLite baza.
**Poznat, još neriješen gap**: `GenerateSocialPost` i dalje ne provjerava da je plan `APPROVED`
prije generisanja posta (dokumentovano u ACS-F1-014, namjerno ostavljeno da se izbjegne fajl-
konflikt — sad kad je ACS-F1-014 mergovan, ovaj mali guard je slobodan da se doda kao prvi
kandidat za novi `FLOW-NNNN` task). Nijedan kontrakt trenutno OPEN. Kandidati: **taj mali guard
task** (dodaj `plan.status == APPROVED` provjeru u `GenerateSocialPost`), **budući "Content
revisions" task** (plan sekcija 38, `revise_content_piece.py`), **A8 — live provider adapter**
(i dalje odgođen po Human Owner odluci, 2026-09-02). **Sljedeći NOVI task dobija `FLOW-1000`**
(§31 workflow dokumenta). GUI dizajn iteracija (vidi ispod) je paralelan, nezavisan trak — čeka
MiniMax-ov trenutni popravak prije nastavka. **ACS-GUI-003** (campaign workflow ekrani) i dalje
čeka da application use-caseovi budu dovoljno razvijeni da ih GUI stvarno poziva.
