...
**Zadnje ažurirano:** 2026-09-01 (coordinator: claude) — ACS-P0-007 Codex round 2 REJECT (R2-BF-1), fix round 3 riješen i verifikovan, čeka Codex round 3
...
NOT DONE — ACS-P0-001 do ACS-P0-006 (6 od 8 P0 taskova) su merged.
...
| ACS-P0-007 | **Fix round 3 riješen — čeka Codex round 3** | Pi | Codex, Claude (HIGH) | Scope: P0.20–P0.23 (Jobs + Presentation contracts + Bootstrap wiring). Contract: `agent_reports/ACS-P0-007-task-contract.md`. Grana `task/ACS-P0-007-jobs-presentation-bootstrap` @ `3a5a5c0` (nije još merged). Tok: Pi implementacija (`de88720`) → fix round 1 (`b687187`, guard prefix-match dead code) → Claude review PASS → **Codex round 1 REJECT** (BF-1 orphan PENDING job nakon submit-after-shutdown, BF-2 dynamic-import guard bypass) → fix round 2 (`ffa25ad`, oba riješena, nezavisno reprodukovana od koordinatora) → **Codex round 2 REJECT** (`agent_reports/2026-09-01-ACS-P0-007-rereview-codex.md`): potvrdio BF-1/BF-2 CLOSED, ali otkrio nov nalaz R2-BF-1 (queued/prihvaćen job ostaje trajno PENDING kad `shutdown(cancel_futures=True)` otkaže njegov not-yet-started Future prije nego što worker stigne da ga pokrene — reprodukovano i od koordinatora prije fix-runde). Fix round 3 (`a9adc76`): `JobManager` sada prati `job_id → Future` mapping, `shutdown()` poslije `executor.shutdown()` skenira otkazane future-e i terminalizuje odgovarajuće jobove kao CANCELLED. Koordinator nezavisno reprodukovao original bug i fix, slomio fix DRUGAČIJE od implementera (isključio Future-tracking umjesto shutdown-cleanup poziva) da potvrdi da regression test stvarno hvata regresiju. 170 testova, ruff/mypy čisto. Non-blocking napomena od Codex-a (double-indirection dynamic import `m = importlib; m.import_module(...)`) eksplicitno ostavljena neriješena po njegovoj sopstvenoj preporuci. Codex round 3 request pripremljen i pushovan (`3a5a5c0`, `agent_reports/2026-09-01-ACS-P0-007-codex-review-request-round3.md`). Čeka Codex rezultat. |
...
Uspostavljen na `main` poslije merge-a ACS-P0-005 + file-header Faze 1 (2026-09-01, root `.venv`,
Python 3.14.1):
