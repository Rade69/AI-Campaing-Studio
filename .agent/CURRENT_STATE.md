...
**Zadnje ažurirano:** 2026-09-01 (coordinator: claude) — ACS-HOTFIX-001 Claude review PASS, čeka Codex; ACS-P0-008 čeka MiniMax-ov fix round
...
|---|---|---|---|---|
| ACS-HOTFIX-001 | **Claude review PASS — čeka Codex** | MiniMax | Codex, Claude (HIGH) | Grana `hotfix/ACS-HOTFIX-001-job-event-ordering` @ `11d5b48`. Fix: `threading.Lock()` → `RLock()`, `CREATED` emit pomjeren unutar `submit()`-ovog lock bloka, `_emit()` sad drži lock kroz cio callback dispatch. Novi deterministički test (slow-callback adversarial probe, ne probabilistički) — dokazano da je upravo probabilistički pristup propustio bug tri runde zaredom. Koordinator tokom verifikacije greškom obrisao MiniMax-ov necommit-ovan test preko lošeg `git stash`/`checkout` manevra — MiniMax ga ponovo dodao, potvrđeno. Koordinator zatim nezavisno otkrio da fix ima redundantnu zaštitu (bilo koja dva od tri elementa fix-a su samostalno dovoljna) — ne defekt, ali ispravlja evidence report-ovu "sve tri neophodne" tvrdnju. 171 test, ruff/mypy čisto, 20x targeted loop čist. Claude review PASS; Codex review request pripremljen i pushovan. |
