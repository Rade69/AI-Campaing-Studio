...
| ACS-P0-002 | **REJECTED round 4 (Codex) — class-scope leak u method resolution, fix round 4 u toku, NE MERGE** | Pi | Codex, Claude | HEAD i dalje `f30c5b3`. Codex round 4: `agent_reports/2026-08-31-ACS-P0-002-review-codex-round4.md` — round-3 cross-function fix i nested closures rade ispravno, ali scanner tretira class body kao enclosing scope za svoje metode (nije tačna Python semantika — class scope se preskače pri method name resolution). Koordinator potvrdio dvostruko: test PROLAZI kad ne bi trebalo, I runtime dokaz (`Evil().method()` sa `math` targetom) potvrđuje da stvaran Python koristi module-level alias, ne class-level. Codex je eksplicitno primijenio proporcionalnost i obrazložio zašto OVO ostaje blocking (false negative na izvršivom kodu, ne teoretski slučaj). Fix round 4 brief: `agent_reports/2026-08-31-ACS-P0-002-fix-round4-brief.md` — traži scope-kind tagging (class frames se preskaču pri upward resolution iz function/method scope-a). |
