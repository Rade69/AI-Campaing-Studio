...
| ACS-P0-005 | **REJECTED (Codex) — fix runda u toku, NE MERGE** | Pi | Codex, Claude | HEAD i dalje `5517c8b`. Codex review: `agent_reports/2026-09-01-ACS-P0-005-review-codex.md` — 3 blocking findings (`security: REJECT`), sva potvrđena nezavisno: **BF-1** `KeyringSecretStore` čuva backend exception kao `__cause__` — ako backend echo-uje secret u svojoj grešci, `exc.__cause__`/traceback ga otkriva iako `str(exc)` ne; **BF-2** `secret_to_env_var()` ne validira naming konvenciju — alias oblik bez `provider/` prefiksa (`"OPENAI/api_key"`) mapira na ISTI env var kao kanonski `"provider/OPENAI/api_key"`, collision; **BF-3** `register_manual_model()`/`register_discovered_models()` ne provjeravaju da provider postoji — model za `"UNKNOWN"` provider je prihvaćen bez greške. Fix brief: `agent_reports/2026-09-01-ACS-P0-005-fix-round-brief.md`. HIGH/security task — ostaje na punom Codex+Claude+Human Owner ciklusu po novoj review politici (vidi vrh fajla). |
| ACS-P0-006 | BF-1/2 FIX DONE (coordinator-confirmed), čeka Codex re-review + Human Owner approval | Crush | Codex, Claude | Novi HEAD `8d45167` (prošli REJECT bio na `92f3917`). Fix dirao 4 fajla. Koordinator nezavisno ponovio oba originalna Codex probe scenarija — oba sad ispravno rade. Pun set 104 testa zelen. Evidence: `agent_reports/2026-09-01-ACS-P0-006-fix-round-crush-confirmed.md`. Codex re-review brief: `agent_reports/2026-09-01-ACS-P0-006-codex-rereview-request.md`. HIGH task — ostaje na punom ciklusu po review politici (vidi vrh fajla). |
