...
| ACS-P0-006 | **OPEN — contract spreman, worktree+branch kreirani, čeka implementaciju (Crush)** | Crush | Codex, Claude (elevated §4 — SQLite/migrations) | Scope: P0.16–P0.19. Kontrakt: `agent_reports/ACS-P0-006-task-contract.md`. Worktree `../ai-campaign-studio-worktrees/ACS-P0-006-sqlite-foundation`, branch `task/ACS-P0-006-sqlite-foundation` od `main@820bbf9`. Paralelno sa 005. `adversarial_required: true` (migration checksum-mismatch + failure-rollback invarijante). Napomena: P0.19 "svih 5 portova" checkpoint ne blokira ni 005 ni 006 pojedinačno — postojeći architecture-boundary test iz 002 već pokriva `ports/` sloj generički. |
